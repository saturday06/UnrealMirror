...
ULoadVrmPathComponent::FStaticOnDropFiles ULoadVrmPathComponent::StaticOnDropFilesDelegate;
ULoadVrmPathComponent *ULoadVrmPathComponent::s_LatestActiveComponent = nullptr;

ULoadVrmPathComponent::ULoadVrmPathComponent(const FObjectInitializer &ObjectInitializer) : Super(ObjectInitializer)
{
}
...
void ULoadVrmPathComponent::OnRegister()
{
	...
	StaticOnDropFilesDelegate.AddUObject(this, &ULoadVrmPathComponent::OnDropFilesDelegate_Handler);
...
bool ULoadVrmPathComponent::VRMGetOpenFileName(FString &FileName)
{
...
#if PLATFORM_WINDOWS
	{
		...
		ofn.lpstrFilter = TEXT("VRM file(*.vrm)\0*.vrm\0") TEXT("all file(*.*)\0*.*\0\0");
